...
const App: React.FC = () => {
  ...
  const renderHours = useMemo(() => {
    return () => Array.from({ length: 24 }).map((_, i) => (
      ...
      >
        <span className="absolute left-2 top-2 text-sm text-gray-500">{dayjs().hour(i).minute(0).format('h A')}</span>
      </div>
    ));
  ...
  }, [events, userSelectedDate]); // Dependency array: events and userSelectedDate

  const formattedTime = currentTime.format('h:mm A'); // Format the time
